...
CREATE INDEX idx_early_users_email ON early_users(email);

-- Enable Row Level Security (RLS)
ALTER TABLE early_users ENABLE ROW LEVEL SECURITY;

-- Create a policy to allow anyone to insert (for signups)
...
-- Optional: Create a policy to allow public reads (if you want to show signup count publicly)
